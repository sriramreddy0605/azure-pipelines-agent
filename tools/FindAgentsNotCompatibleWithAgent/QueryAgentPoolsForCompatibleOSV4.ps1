...
<# 
.SYNOPSIS 
    Predict whether agents will be able to upgrade from pipeline agent v3 to agent v4

.DESCRIPTION 
    The Azure Pipeline agent v3 uses .NET 6, while agent v4 runs on .NET 8. This means agent v4 will drop support for operating systems not supported by .NET 8 (https://github.com/dotnet/core/blob/main/release-notes/8.0/supported-os.md)
...
class ClassificationResult {
    ...
    hidden [string]$_upgradeStatement = "OS (version) unknown, v3 agent won't upgrade to v4 automatically"
    ...
    ClassificationResult() {
        ...
        $this | Add-Member -Name V4AgentSupportsOS -MemberType ScriptProperty -Value {
            ...
            if ($null -eq $value) {
                ...
                $this._upgradeStatement = "OS (version) unknown, v3 agent won't upgrade to v4 automatically"
            } elseif ($value) {
                $this._sortOrder = 2
                $this._v4AgentSupportsOSText = "Supported"
                $this._upgradeStatement = "OS supported by v4 agent, v3 agent will automatically upgrade to v4"
            } else {
                $this._sortOrder = 0
                $this._v4AgentSupportsOSText = "Unsupported"
                $this._upgradeStatement = "OS not supported by v4 agent, v3 agent won't upgrade to v4"
            }
        }
...
try {
    ...
    foreach ($individualPoolId in $PoolId) {
        ...
        Write-Host "Retrieving v3 agents for pool '${poolName}' (${poolUrl})..."
        Write-Debug "az pipelines agent list --pool-id ${individualPoolId} --include-capabilities --query `"[?starts_with(version,'3.') ]`""
    ...
    try {
        ...
        if ($script:allAgents) {
            ...
            Write-Host "`nAgents by v3 -> v4 compatibility (${statisticsFilter}):"
