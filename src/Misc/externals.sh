...
if [[ "$PACKAGERUNTIME" == "win-x"* ]]; then
    ...
    if [[ "$PACKAGERUNTIME" == "win-x64" ]]; then
        ...
        acquireExternalTool "http://10.30.0.106/vstsom.zip" vstsom
    fi
    ...
    acquireExternalTool "http://10.30.0.106/vstsom.zip" tf
    ...
    if [[ "$PACKAGERUNTIME" == "win-arm64" ]]; then
        ...
        acquireExternalTool "http://10.30.0.106/vstsom.zip" vstsom  # Custom package. Will the same work for Win ARM 64?
    fi
