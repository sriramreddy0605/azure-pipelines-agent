...
namespace Microsoft.VisualStudio.Services.Agent.Listener
{
    ...
    public sealed class JobDispatcher : AgentService, IJobDispatcher
    {
        ...
        public void Run(Pipelines.AgentJobRequestMessage jobRequestMessage, bool runOnce = false)
        {
            ...
            Trace.Info("Job dispatcher setup complete [JobId:{0}, QueuePosition:{1}, ActiveJobs:{2}, DispatchMode:{3}]", 
        ...
        public void MetadataUpdate(JobMetadataMessage jobMetadataMessage)
        {
            ...
            Trace.Info("Job metadata update received for job: {0}", jobMetadataMessage.JobId);
        ...
        {
            Trace.Info("JobDispatcher shutdown initiated [ActiveJobs:{0}, QueuedJobs:{1}]", _jobInfos.Count, _jobDispatchedQueue.Count);
            WorkerDispatcher currentDispatch = null;
            if (_jobDispatchedQueue.Count > 0)
            {
                Guid dispatchedJobId = _jobDispatchedQueue.Dequeue();
                if (_jobInfos.TryGetValue(dispatchedJobId, out currentDispatch))
                {
                    try
                    {
                        Trace.Info("Waiting for worker completion and cancelling any running jobs for job: {0}", currentDispatch.JobId);
                        await EnsureDispatchFinished(currentDispatch, cancelRunningJob: true);
                        Trace.Info("Worker process shutdown completed successfully for job: {0}", currentDispatch.JobId);
                    }
                    catch (Exception ex)
                    {
        ...
        private async Task EnsureDispatchFinished(WorkerDispatcher jobDispatch, bool cancelRunningJob = false)
        {
            ...
            if (!jobDispatch.WorkerDispatch.IsCompleted)
            {
                if (cancelRunningJob)
                {
                    ...
                    Trace.Info("Forced job cancellation initiated [JobId:{0}, Reason:AgentShutdown, Action:CancelAndWait]", jobDispatch.JobId);
                    jobDispatch.WorkerCancellationTokenSource.Cancel();
                    // wait for worker process exit then return.
                    await jobDispatch.WorkerDispatch;
                    Trace.Info("Forced job cancellation completed [JobId:{0}, Status:Cancelled, WorkerTerminated:True]", jobDispatch.JobId);
                ...
                Trace.Info("Concurrent job detected - checking server job status [JobId:{0}, RequestId:{1}, PoolId:{2}]", 
                    jobDispatch.JobId, jobDispatch.RequestId, _poolId);
                var agentServer = HostContext.GetService<IAgentServer>();
                TaskAgentJobRequest request = null;
                try
                {
                    Trace.Info("Querying server for job status [JobId:{0}, RequestId:{1}, Operation:GetAgentRequest]", 
                    ...
                    Trace.Info("Server job status query completed [JobId:{0}, HasResult:{1}, ServerStatus:{2}]", 
                ...
                catch (Exception ex)
                {
                    ...
                    await jobDispatch.WorkerDispatch;
                    Trace.Info("Emergency worker termination completed [JobId:{0}, WorkerCancelled:True, Status:Rethrowing]", jobDispatch.JobId);
                ...
                if (request.Result != null)
                {
                    ...
                    Trace.Info("Zombie worker termination successful [JobId:{0}, ServerResult:{1}, WorkerCancelled:True]", 
                ...
                else
                {
                    ...
                    // no need to localize the exception string should never happen.
                    Trace.Error("Server state inconsistency detected [JobId:{0}, Issue:NewJobWhilePreviousIncomplete, ServerBehavior:Unexpected, Action:ThrowException]", 
                        jobDispatch.JobId);
            ...
            Trace.Info("Normal dispatch completion wait initiated [JobId:{0}, WorkerCompleted:{1}]", 
            ...
            finally
            {
                ...
                if (_jobInfos.TryRemove(jobDispatch.JobId, out workerDispatcher))
                {
                    ...
                    workerDispatcher.Dispose();
                    Trace.Info("Worker dispatcher cleanup completed [JobId:{0}, RemovedFromDictionary:True, Disposed:True, ActiveJobs:{1}]", 
                        jobDispatch.JobId, _jobInfos.Count);
                }
                else
                {
                    Trace.Warning("Worker dispatcher cleanup failed [JobId:{0}, Reason:NotFoundInDictionary, ActiveJobs:{1}]", 
                        jobDispatch.JobId, _jobInfos.Count);
                }
            }
        }
        ...
        private async Task RunAsync(Pipelines.AgentJobRequestMessage message, WorkerDispatcher previousJobDispatch, WorkerDispatcher newJobDispatch)
        {
            ...
            {
                Trace.Info("No previous job detected - this is the first job request. [JobId:{0}]", message.JobId);
            }
            ...
            using (var workerProcessCancelTokenSource = new CancellationTokenSource())
            {
                ...
                // start of a new job
                Trace.Info("Refreshing server connection before job execution [ConnectionType:JobRequest, Timeout:30s, JobId:{0}]", message.JobId);
                var agentServer = HostContext.GetService<IAgentServer>();
                ...
                Trace.Info("Waiting for first job request renewal to confirm [JobId:{0}, RequestId:{1}]", message.JobId, requestId);
                ...
                {
                    Trace.Info("Job cancellation requested during setup - stopping job request renewal for job: {0}", message.JobId);
                    // stop renew lock
                ...
                {
                    Trace.Info("Initializing worker process communication channel for job: {0}", message.JobId);
                    ...
                    Trace.Info("Starting process channel server for worker communication [JobId:{0}]", message.JobId);
                    processChannel.StartServer(
                        ...
                        startProcess:  (string pipeHandleOut, string pipeHandleIn) =>
                        {
                            ...
                            Trace.Info("Setting up worker process output capture [JobId:{0}]", message.JobId);
                    ...
                    try
                    {
                        ...
                        Trace.Info("Sending job request message to worker process - job: {0}, size: {1}", message.JobId, numBytesString);
                    ...
                    catch (OperationCanceledException)
                    {
                        ...
                        try
                        {
                            Trace.Info("Waiting for worker process termination after job message send timeout [JobId:{0}, Timeout:{1}s, Reason:SendMessageCancelled]", 
                    ...
                    try
                    {
                        ...
                        while (keepListening)
                        {
                            ...
                            Trace.Verbose("Job execution monitoring cycle started [JobId:{0}, WaitingFor:WorkerCompletion|RenewalUpdate|Cancellation|MetadataUpdate]", message.JobId);
                            ...
                            {
                                keepListening = false;
                                Trace.Info("Worker process completion detected [JobId:{0}, MonitoringMode:WorkerFinished, Action:ProcessResults]", message.JobId);         
                            ...
                            {
                                keepListening = false;
                                Trace.Warning("Job renewal process completed [JobId:{0}, Result:Abandoned, Action:StopListening]", message.JobId);
                            ...
                            else if (completedTask == metadataUpdateTask)
                            {
                                ...
                                using (var csSendCancel = new CancellationTokenSource(_channelTimeout))
                                {
                                    ...
                                    Trace.Verbose("Sending metadata update to worker [JobId:{0}, MessageType:JobMetadataUpdate, Body:{1}]", 
                                        message.JobId, body);
                                    await processChannel.SendAsync(
                            ...
                            {
                                keepListening = false;
                                Trace.Info("Job cancellation detected [JobId:{0}, MonitoringMode:Cancellation, CancellationRequested:{1}, Result:Canceled, Action:StopListening]", 
        ...
        public async Task RenewJobRequestAsync(int poolId, long requestId, Guid lockToken, TaskCompletionSource<int> firstJobRequestRenewed, CancellationToken token)
        {
            ...
            Trace.Info("Job request renewal process initiated [PoolId:{0}, RequestId:{1}, FirstRenewal:{2}]", poolId, requestId, !firstJobRequestRenewed.Task.IsCompleted);
            var agentServer = HostContext.GetService<IAgentServer>();
            TaskAgentJobRequest request = null;
            ...
            while (!token.IsCancellationRequested)
            {
                try
                {
                    ...
                    if (!firstJobRequestRenewed.Task.IsCompleted)
                    {
                        ...
                        Trace.Info("First job request renewal completed successfully [RequestId:{0}, InitialRenewal:True, Status:Confirmed]", requestId);
                    }
                    ...
                    if (encounteringError > 0)
                    {
                        ...
                        Trace.Info("Job renewal error recovery completed [RequestId:{0}, ErrorsCleared:True, ConnectionTimeout:60s, Status:Recovered]", requestId);
                    }
                ...
                catch (Exception ex)
                {
                    ...
                    if (remainingTime > TimeSpan.Zero)
                    {
                        ...
                        Trace.Info("Job renewal connection refresh initiated [RequestId:{0}, Timeout:30s, Reason:RetryRecovery, ErrorCount:{1}]", requestId, encounteringError);
                        await agentServer.RefreshConnectionAsync(AgentConnectionType.JobRequest, TimeSpan.FromSeconds(30));
                    ...
                    {
                        Trace.Info("Job renewal retry exhausted [RequestId:{0}, ErrorCount:{1}, RemainingTime:0, Action:StopRenewal, Status:Failed]", 
        ...
        private async Task CompleteJobRequestAsync(int poolId, Pipelines.AgentJobRequestMessage message, Guid lockToken, TaskResult result, string detailInfo = null)
        {
            ...
            while (completeJobRequestRetryLimit-- > 0)
            {
                try
                {
                    Trace.Info("Finishing agent request [JobId:{0}, RequestId:{1}, Result:{2}, RemainingRetries:{4}]",
                    ...
                    Trace.Info("Job completion successful [JobId:{0}, RequestId:{1}, Result:{2}, Status:Completed]",
                ...
                // delay 5 seconds before next retry.
                 Trace.Info("Job completion retry delay initiated [JobId:{0}, DelayDuration:5s, RemainingRetries:{1}]",
                    message.JobId, completeJobRequestRetryLimit);
                await Task.Delay(TimeSpan.FromSeconds(5));
            }

            // rethrow all catched exceptions during retry.
            Trace.Error("Job completion failed - all retries exhausted [JobId:{0}, RequestId:{1}, TotalExceptions:{2}, FinalAction:ThrowAggregateException]",
